...
!!! success "Live Schema Sample"
    `pipeline/compliance.py` writes one JSON object per line to `events.jsonl`, file-drop transport. ==This is not the full external-system contract envisioned below (no HTTP/queue transport, no evidence images, privacy unaddressed)==, but it is real, working, and the right thing to build a first dashboard integration against.
...
Where it lives: `runs/compliance/<name>/events.jsonl`, alongside `compliance_summary.json` (counts by violation type, average duration, the thresholds used).
